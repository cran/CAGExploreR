...
To install \textbf{CAGExploreR}, you need to have R version 3.0.2 or newer (\url{http://www.r-project.org/}). You can download the package from \url{https://github.com/edimont/CAGExploreR/} into a local directory (we'll assume that this directory is \textit{D:/Downloads}, but this can be anything). There is one file called \textbf{CAGExploreR\_1.0.tar.gz} (main source code). There is no need to unpack or decompress the contents of this file. Next open R and run the following code (you only have to do this once). Since \textbf{CAGExploreR} depends on several other packages, you will need to install them first. You may be asked if you want to update some previously installed packages but this does not affect \textbf{CAGExploreR} in any way. When downloading from \textbf{CRAN} you may be asked to choose a mirror to download from. Any selection is fine. 
<<repos, eval=FALSE>>=
...
Once the dependencies are installed, we can now install the package from source. Be sure to set the working directory to the one that contains the downloaded package files. 
...
To start the download, run the following code, it will automatically get all the necessary data from the \textbf{ENSEMBL} biomart:
...
\item \textbf{dominant.promoter.switch} shows whether the dominant promoter switches between conditions. This is in the form of a pipe (|) delimited list of promoters if there is a switch in dominant promoters and is empty otherwise.
